...
using TinyMessengerTests.TestData;
...
namespace TinyMessenger.Tests
{
    ...
    public class TinyMessengerTests
    {
        ...
        [TestMethod]
        public async Task PublishAsync_NoCallback_PublishesMessageAwait()
        {
            var messenger = UtilityMethods.GetMessenger();
            bool received = false;
            ...
            watcher.Stop();
            Assert.AreEqual(1000, watcher.ElapsedMilliseconds, 10);
            Assert.IsTrue(received);
        }

        [TestMethod]
        public void PublishAsync_NoCallback_PublishesMessageNotAwait()
        {
            var messenger = UtilityMethods.GetMessenger();
            bool received = false;
            messenger.Subscribe<TestMessage>((m) =>
            ...
            messenger.PublishAsync(new TestMessage(this));
            watcher.Stop();
            Assert.AreEqual(0, watcher.ElapsedMilliseconds, 10);
